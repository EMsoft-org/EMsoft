...
if [ -e "$SDK_INSTALL/$bcls_FOLDER_NAME" ];
then
	rm -rf "$SDK_INSTALL/$bcls_FOLDER_NAME"
fi
...
mkdir Build
cd Build
${SDK_INSTALL}/${CMAKE_FOLDER_NAME}/bin/cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$SDK_INSTALL/${bcls_INSTALL_NAME}-${bcls_VERSION}-Debug ../
...
mkdir zRel
cd zRel
${SDK_INSTALL}/${CMAKE_FOLDER_NAME}/bin/cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$SDK_INSTALL/${bcls_INSTALL_NAME}-${bcls_VERSION}-Release ../
make -j$PARALLEL_BUILD
make install
