...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  5 07:29:09 2019

@author: joseph
@brief: Take in a CSV that contains DDD or MD simulation results and output a 4D presorted array
in h5 format.
"""
...
"""
NB: The EMdddSTEM program assumes that the grid will be uniformly spaced and indexed. As a result,
the posscale parameter must be set such that the points where the displacement vector is defined lie 
at integer coordinates. 

If your grid is not defined such that 
"""

rotate  = True            # does the data need to be rotated
propdir = 'Z'            # direction of beam propagation
posscale= 1              # scale point grid
disscale= 1              # scale of displacement values, if not in nm

frac    = 1              # Lower the resolution of the grid, which fraction of points are included - generally this is not needed
...
# This script assumes that the 2D array is formatted as follows: (x,y,z,dx,dy,dz)
