...
; function name modified to align with usage in EMsoft IDL package [M. De Graef, 12/21/18]

function Core_Tag_Exists, str, tag,index=index, top_level=top_level,recurse=recurse, $
         quiet=quiet
 ...
 if ~nmatch && ~keyword_set(top_level) then begin
       ...
       for i=0,n_elements(tn)-1 do begin
        if size(str.(i),/TNAME) eq 'STRUCT' then $
                status=Core_Tag_Exists(str.(i),tag,index=index)
